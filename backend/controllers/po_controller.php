...
function get_po_orders($conn, $limit, $offset, $filter, $filterParams)
{
    try {
        $query = "
            ...
            JOIN po_orders_status pos ON po.po_order_status_id = pos.id
            $filter
            ORDER BY po.po_order_id DESC
...
function get_po_orders_items($conn, $po_order_id)
{
    try {
        $query = "
            SELECT 
...
function get_po_orders_details($conn, $po_order_id)
{
    try {
        $query = "
            SELECT
            ...
            WHERE po.po_order_id = :po_order_id;
            ";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':po_order_id', $po_order_id);
        $stmt->execute();
        ...
        $order['items'] = json_decode(get_po_orders_items($conn, $po_order_id), true);
