...
const generateBarcode = async (order, apiHost, accessToken) => {
  ...
  const address = details.override_address
  ...
  if (haveEmail) {
    ...
    ship_country = AddressController.getValueByIndex(
    ...
    receiver_address = addressSplit[1]
  ...
  const newItem = {
    ...
    receiver: {
      name: details.override_address
        ? addressSplit[0][0].toUpperCase()
        : details.buyer_name
        ? details.buyer_name.toUpperCase()
        : "",
      tel: buyer_phone_number
      ...
      address: {
        ...
        countryCode: details.ship_country
...
const uploadFile = async (fileUrl, orderId) => {
  try {
    ...
    const file = new File([blob], `label-${orderId}.pdf`, {
    ...
    const updateOrderFile = await DataController.insert(
      "order_files",
      to_insert_file
    );
