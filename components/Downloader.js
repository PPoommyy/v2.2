...
const readExcel = async (fileName) => {
  try {
    const response = await axios.get(
      `../../backend/file/read_file.php?fileName=${fileName}`,
      {
...
const getStyle = (cellRef, sheet) => {
  ...
  const width =
    cell && sheet.getColumn(columnNumber).width
...
const duplicateStyleByRange = (
  ...
  for (let i = startIndex; i < lastIndex; i++) {
    for (let char of generateColumnRange(startCell, lastCell)) {
      ...
      const outputCell = worksheet.getCell(
        currentIndex + i + 1,
        columnToNumber(char)
      );
      ...
      if (productSetMap[item.order_product_sku]) {
        ...
        setItems.forEach((setItem) => {
          const itemPricePerUnit =
            setItemPrice / (setItem.quantity * item.quantity_purchased);
          newItems.push({
            ...setItem,
...
const generateOrderExcel = async (orders, toggleSpinner, checkboxStates) => {
  try {
    ...
    selectedOrders.forEach((order) => {
      ...
      const addressSplit = raw_address
        .split("\n")
        .map((line) => line.toUpperCase());
      const orderNoteSplit = order_note
      ...
      for (let sliceIndex = 0; sliceIndex < orderSliceNumber; sliceIndex++) {
        ...
        if (sliceIndex === 0 && sliceIndex < orderSliceNumber - 1) {
          ...
          addressSplit.forEach((line) => {
            ws.getCell(orderIndex + addressIndex, columnToNumber("B")).value =
              line;
            addressIndex++;
          });

          ws.getCell(orderIndex + 1, columnToNumber("I")).value = timesort;
          ws.getCell(orderIndex + 1, columnToNumber("J")).value =
            payments_date.split(" ")[0];
          ws.getCell(orderIndex + 1, columnToNumber("K")).value =
            addressSplit.join();
          ws.getCell(orderIndex + 1, columnToNumber("L")).value = currency_code;
          ws.getCell(orderIndex + 1, columnToNumber("M")).value =
            all_total.toFixed(2);
            ...
            richText: [
              {
                ...
                text: `${payment_methods} (${website_name})`,
              },
              {
                font: {
                  size: 10,
          ...
          ws.getCell(
            footerIndex,
            columnToNumber("D")
          ).value = `Discount: ${ship_promotion_discount}`;
          ws.getCell(footerIndex, columnToNumber("G")).value = currency_code;
          ws.getCell(footerIndex, columnToNumber("H")).value =
            all_total.toFixed(2);
        ...
        ws.getCell(
          orderIndex + 1,
          columnToNumber("C")
        ).value = `No. ${timesort} (${payments_date.split(" ")[0]})`;
        ...
        slicedItems.forEach((item) => {
          ...
          const shippingPrice =
            itemIndex === 1 && sliceIndex === 0 ? shipping_fee : shipping_price;
          const subTotal =
            parseFloat(item_price * quantity_purchased) +
            parseFloat(shippingPrice);
          const productNameRichText = [
            {
              text: report_product_name,
              font: { color: { argb: "FF000000" } },
            },
          ];

          if (quantity_purchased > 1) {
            productNameRichText.push({
          ...
          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("C")).value = {
            richText: productNameRichText,
          };
          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("D")).value =
            sku;
          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("E")).value =
            item_price;
          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("F")).value =
            quantity_purchased;

          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("G")).value =
            shippingPrice;
          ws.getCell(orderIndex + itemIndex + 1, columnToNumber("H")).value =
            subTotal;

          total +=
            item_price * quantity_purchased + shippingPrice / orderSliceNumber;
          itemIndex++;
          if (
            orderNoteSplit &&
            itemIndex - 1 === slicedItems.length &&
            sliceIndex === orderSliceNumber - 1
          ) {
            if (
              6 - itemIndex < orderNoteSplit.length &&
              orderSliceNumber != 1
            ) {
              let orderNoteIndex = 0;
              orderNoteSplit.forEach((orderNote) => {
                ws.getCell(
                  orderIndex + orderNoteIndex + 1,
                  columnToNumber("B")
                ).value = {
                  richText: [
                    {
                      text: orderNote,
                      font: { color: { argb: "FFFF0000" }, name: "Calibri" },
                    },
                  ],
                };
                orderNoteIndex++;
    ...
    selectedOrders.forEach((order) => {
      ...
      for (let sliceIndex = 0; sliceIndex < orderSliceNumber; sliceIndex++) {
        ...
        if (sliceIndex === 0 && sliceIndex < orderSliceNumber - 1) {
          ...
          ws.getCell(orderIndex + 1, columnToNumber("L")).value =
            all_total.toFixed(2);
          ws.getCell(orderIndex + 1, columnToNumber("N")).value =
            tax_rate.tax_rate * 100;
          ws.getCell(orderIndex + 1, columnToNumber("AM")).value =
            addressSplit.join();
        } else if (sliceIndex === 0) {
          for (let i = 0; i < 7; i++) {
            for (let char of generateColumnRange("A", "AM")) {
          ...
          ws.getCell(orderIndex + 1, columnToNumber("L")).value =
            all_total.toFixed(2);
          ws.getCell(orderIndex + 1, columnToNumber("N")).value =
            tax_rate.tax_rate ? tax_rate.tax_rate * 100 : 0;
          ws.getCell(orderIndex + 1, columnToNumber("AM")).value =
            addressSplit.join();
          ...
          ws.getCell(footerIndex, columnToNumber("E")).value =
            ship_promotion_discount;
          ws.getCell(footerIndex, columnToNumber("G")).value = currency_code;
          ws.getCell(footerIndex, columnToNumber("H")).value =
            all_total.toFixed(2);
...
const generateOrderExcel3 = async (orders, toggleSpinner, checkboxStates) => {
  try {
    ...
    const selectedOrders = orders.filter((order) =>
      checkboxStates.includes(order.details.timesort)
    );
    selectedOrders.sort((a, b) => a.details.timesort - b.details.timesort);
...
const processOrder = (order, ws, templateSheet, orderIndex) => {
  ...
  const orderNoteSplit = details.order_note
    ? details.order_note.split("\n").filter((line) => line.trim())
    : null;
  ...
  for (let sliceIndex = 0; sliceIndex < orderSliceNumber; sliceIndex++) {
    ...
    fillOrderItems(
      ws,
      items.slice(sliceIndex * 5, (sliceIndex + 1) * 5),
      details.shipping_fee,
      orderSliceNumber,
      orderIndex
    );
    if (sliceIndex === orderSliceNumber - 1)
      fillFooter(ws, details, all_total, orderIndex);
    if (orderNoteSplit && sliceIndex === orderSliceNumber - 1)
      fillOrderNotes(ws, orderNoteSplit, orderIndex);
    orderIndex += 7;
  }
  return orderIndex;
};
...
const fillOrderItems = (
  ...
  slicedItems.forEach(
    ({
      ...
      ws.getCell(orderIndex + itemIndex + 1, columnToNumber("C")).value = {
        richText: [
          { text: report_product_name, font: { color: { argb: "FF000000" } } },
        ],
      };
...
const fillFooter = (ws, details, all_total, orderIndex) => {
  ws.getCell(orderIndex + 7, columnToNumber("C")).value = {
    richText: [
      {
        font: { size: 10, name: "Calibri" },
        text: `${details.payment_methods} (${details.website_name})`,
      },
      {
        font: { size: 10, color: { argb: "FFFF0000" }, name: "Calibri" },
        text: details.deposit ? `* มัดจำ ${details.deposit}` : "",
      },
    ],
  };
  ws.getCell(orderIndex + 7, columnToNumber("D")).value = `Discount:`;
  ws.getCell(orderIndex + 7, columnToNumber("E")).value =
    details.ship_promotion_discount;
...
const generateInvoiceExcel = async (orders, toggleSpinner, checkboxStates) => {
  try {
    ...
    selectedOrders.forEach((order) => {
      ...
      const addressSplit = details.raw_address
        .split("\n")
        .map((line) => line.toUpperCase());
      const addressLineCount = addressSplit.length;
      const haveEmail = addressSplit[addressLineCount - 1].indexOf("@") !== -1;
      ...
      ws.getCell(
        41,
        columnToNumber("A")
      ).value = `Currency : ${details.currency_code}`;
    });
...
const generateItemSummaryExcel = async (
  orders,
  toggleSpinner,
  checkboxStates
) => {
  try {
    ...
    const from =
      selectedOrders[selectedOrders.length - 1].details.payments_date.split(
        " "
      )[0];
...
const generateDhlPreAlertExcel = async (
  orders,
  toggleSpinner,
  checkboxStates
) => {
  try {
    ...
    selectedOrders.forEach((order) => {
      ...
      ws.getCell(orderIndex, columnToNumber("B")).value =
        details.buyer_name.toUpperCase();
      ...
      ws.getCell(orderIndex, columnToNumber("J")).value =
        details.ship_phone_number;
...
const generateDpostExcel = async (orders, toggleSpinner, checkboxStates) => {
  try {
    ...
    selectedOrders.forEach((order) => {
      ...
      const resultArray = AddressController.splitAddressData(
        details.raw_address
      ).slice(1);

      const isThai = AddressController.checkThaiProvince(resultArray);

      const addressSplit = AddressController.splitAddressData(
        details.raw_address
      );
      const addressLineCount = addressSplit.length;
      const haveEmail =
        addressSplit[addressLineCount - 1][0].indexOf("@") !== -1;
      ...
      if (haveEmail && addressLineCount > 2) {
        ...
        ship_city = AddressController.getValueByIndex(
          addressSplit[addressLineCount - 4],
          2,
          false
        );
        ship_postal_code = AddressController.getValueByIndex(
          addressSplit[addressLineCount - 4],
          2,
          true
        );
        ship_state = AddressController.getValueByIndex(
          addressSplit[addressLineCount - 3],
          2,
          false
        );
        ...
        ship_state = AddressController.getValueByIndex(
          addressSplit[addressLineCount - 2],
          2,
          false
        );
      } else {
      ...
      const receiver_address = addressSplit[1]
        ? addressSplit[1].length > 0
          ? addressSplit[1].join(" ")
          : addressSplit[1]
        : "";

      ws.getCell(orderIndex, columnToNumber("A")).value = details.ship_country
        ? details.ship_country === "UK"
          ? "GB"
          : details.ship_country
        : "";
      ws.getCell(orderIndex, columnToNumber("C")).value = service_id
        ? service_id.service_id
        : "";
      ws.getCell(orderIndex, columnToNumber("N")).value = details.buyer_name
        ? details.buyer_name.toUpperCase()
        : "";
      ws.getCell(orderIndex, columnToNumber("O")).value = buyer_phone_number
        ? AddressController.formatPhoneNumber(buyer_phone_number)
        : "";
      ws.getCell(orderIndex, columnToNumber("P")).value = haveEmail
        ? addressSplit[addressLineCount - 1][0]
        : "";
      ws.getCell(orderIndex, columnToNumber("Q")).value = receiver_address;
      ws.getCell(orderIndex, columnToNumber("R")).value = ship_state
        ? ship_state
        : "";
      ws.getCell(orderIndex, columnToNumber("S")).value = ship_city
        ? ship_city
        : "";
      ws.getCell(orderIndex, columnToNumber("T")).value = ship_postal_code
        ? ship_postal_code
        : "";
      ws.getCell(orderIndex, columnToNumber("U")).value = details.currency_code
        ? details.currency_code
        : "";
      ws.getCell(orderIndex, columnToNumber("Y")).value = sub_total
        ? sub_total.toFixed(2)
        : "";
      orderIndex++;
    });
...
const generateMergedPDF = async (selectedOrders) => {
  ...
  for (const order of selectedOrders) {
    ...
    try {
      const existingPdfBytes = await fetch(labelUrl).then((res) =>
        res.arrayBuffer()
      );
      ...
      console.error(
        `Error merging label for order ${order.details.order_id}:`,
        error
      );
    }
  }
