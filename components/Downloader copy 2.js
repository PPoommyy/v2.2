const readExcel = async (fileName) => {
  try {
    const response = await axios.get(
      `../../backend/file/read_xlsx.php?fileName=${fileName}`,
      {
...
const checkThaiProvince = (addressArray) => {
  ...
  const provinceNames = addressArray.filter((item) =>
    thaiProvinceArray.flat().includes(item)
  );
      ...
      for (let sliceIndex = 0; sliceIndex < orderSliceNumber; sliceIndex++) {
        ...
        if (sliceIndex === 0 && sliceIndex < orderSliceNumber - 1) {
          ...
          ws.getCell(
            footerIndex,
            columnToNumber("C")
          ).value = `${payment_methods} (${website_name})`;
          ws.getCell(
            footerIndex,
            columnToNumber("D")
        ...
        slicedItems.forEach((item) => {
          ...
          const productNameRichText = [
            {
              ...
              font: { color: { argb: "FF000000" }, name: "Calibri" },
            },
          ];
          ...
          if (quantity_purchased > 1) {
            productNameRichText.push({
              ...
              font: { size: 9, color: { argb: "FFFF0000" }, name: "Calibri" },
            });
          }
          ...
          if (
            ...
            if (5 - itemIndex < orderNoteSplit.length) {
              ...
              ws.getCell(orderIndex, columnToNumber("B")).font = {
                color: { argb: "FF000000" },
              };
            } else {
              orderNoteSplit.forEach((orderNote) => {
...
const generateDpostExcel = async (orders, toggleSpinner, checkboxStates) => {
  try {
    ...
    selectedOrders.forEach((order) => {
      ...
      const buyer_phone_number = haveEmail
        ? addressSplit[addressLineCount - 2][0]
        : addressSplit[addressLineCount - 1][0];
      const ship_city = haveEmail
        ? getValueByIndex(addressSplit[addressLineCount - 4], 2, false)
        : getValueByIndex(addressSplit[addressLineCount - 3], 2, false);
      const ship_postal_code = haveEmail
        ? getValueByIndex(addressSplit[addressLineCount - 4], 2, true)
        : getValueByIndex(addressSplit[addressLineCount - 3], 2, true);
      const ship_state = haveEmail
        ? getValueByIndex(addressSplit[addressLineCount - 3], 2, false)
        : getValueByIndex(addressSplit[addressLineCount - 2], 2, false);
      ...
      ws.getCell(orderIndex, columnToNumber("O")).value = buyer_phone_number
        ? formatPhoneNumber(buyer_phone_number)
        : "";
