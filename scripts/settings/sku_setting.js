...
const get_options = async (option) => {
  try {
    const column = ["*"];
...
async function generateTable(limit, page) {
  try {
    ...
    tableHeaderRow.innerHTML = `<th>Order Product SKU</th>
    ...
    dropdownTitle.innerText = `Showing ${limit * (page - 1) + 1}-${
      totalCount < limit ? totalCount : limit * page
    } of ${totalCount} rows`;
...
addButton.addEventListener("click", async (event) => {
  ...
  tableRow.appendChild(
    Cell.createSelectCell(warehouseSkus, "warehouse_sku_id")
  );
...
saveButton.addEventListener("click", async () => {
  ...
  try {
    for (let index = 0; index < newRows.length; index++) {
      ...
      try {
        const result = await DataController.insert(
          "sku_settings",
          insertedData
        );
        results.push(result);
