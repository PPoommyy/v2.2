...
updateButton.addEventListener('click', async () => {
    ...
    if(value) {
        const result = await DataController.updateByKey("warehouse_skus", "id", id, key, value);
...
saveButton.addEventListener('click', async () => {
    ...
    try {
        for (let index = 0; index < newRows.length; index++) {
            ...
            try {
                const result = await DataController.insert("warehouse_skus", insertedData);
                results.push(result);
...
async function get_warehouse_skus(limit, page) {
    try {
        let url = `../../backend/get/get_warehouse_skus.php?limit=${limit}&page=${page}`;
...
async function generateTable(limit, page) {
    try {
        toggleSpinner(true);
        const result = await get_warehouse_skus(limit, page);
        const warehouse_skus = result.data;
        const totalCount = result.count;
        const totalPages = Math.ceil(totalCount / limit);

        const warehouseDataContainer = document.getElementById('warehouse-data-container');
        warehouseDataContainer.innerHTML = '';
        ...
        warehouse_skus.forEach(warehouse_sku => {
            const tableRow = document.createElement('tr');
            ...
            deleteButton.addEventListener('click', async () => {
                ...
                const result = await DataController._delete("warehouse_skus", "id", warehouse_sku.id);
                if (result&&result.status) {
            ...
            tableRow.appendChild(Cell.createInputOnModalCell('Warehouse SKU', warehouse_sku.id, 'name', warehouse_sku.name));
            tableRow.appendChild(deleteButtonCell);
            tableBody.appendChild(tableRow);
        });
        tableElement.appendChild(tableBody);

        warehouseDataContainer.appendChild(tableElement);
