...
async function get_currency_list(limit, page) {
  try {
    let url = `../../backend/get/get_currency_list.php?limit=${limit}&page=${page}`;
...
async function generateTable(limit, page) {
  try {
    ...
    const currencyDataContainer = document.getElementById(
    ...
    tableHeaderRow.innerHTML = `<th>Currency Code</th>
             <th>Description</th>
    ...
    tableBody.id = "currency-data-tbody";
    currencies.forEach((currency) => {
      ...
      switchInput.addEventListener("click", async () => {
        const confirmAlert = await Alert.showConfirmModal(
          `Are you sure you want to ${
            switchInput.checked ? "enable" : "disable"
          } to show on dropdown?`
        );
      ...
      tableRow.appendChild(
        Cell.createInputOnModalCell("Name", currency.id, "name", currency.name)
      );
      tableRow.appendChild(
...
addButton.addEventListener("click", async function (event) {
  ...
  const tbody = document.getElementById("currency-data-tbody");
  const tableRow = document.createElement("tr");
...
saveButton.addEventListener("click", async () => {
  ...
  try {
    for (let index = 0; index < newRows.length; index++) {
      ...
      try {
        const result = await DataController.insert("currencies", insertedData);
        results.push(result);
