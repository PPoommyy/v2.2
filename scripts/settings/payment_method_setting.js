...
updateButton.addEventListener('click', async () => {
    ...
    if (value) {
        const result = await DataController.updateByKey("payment_methods", "id", id, key, value);
...
saveButton.addEventListener('click', async () => {
    ...
    try {
        for (let index = 0; index < newRows.length; index++) {
            ...
            try {
                const result = await DataController.insert("payment_methods", insertedData);
                results.push(result);
...
async function get_payment_methods(limit, page) {
    try {
        let url = `../../backend/get/get_payment_methods.php?limit=${limit}&page=${page}`;
    ...
    try {
        ...
        paymentMethods.forEach(paymentMethod => {
            ...
            deleteButton.addEventListener('click', async () => {
                ...
                const result = await DataController._delete("payment_methods", "id", paymentMethod.id);
                if (result&&result.status) {
            ...
            tableRow.appendChild(Cell.createInputOnModalCell('Name', paymentMethod.id, 'name', paymentMethod.name));
            tableRow.appendChild(deleteButtonCell);
            tableBody.appendChild(tableRow);
        });
        tableElement.appendChild(tableBody);

        paymentDataContainer.appendChild(tableElement);
