...
async function generateTable(limit, page) {
  try {
    ...
    orders.forEach((order) => {
      ...
      if (tracking.length > 0) {
        ...
        envelopeIcon.addEventListener("click", () => {
          ...
          const createTrackingBtn =
            document.getElementById("createTrackingBtn");
          const deleteTrackingBtn =
            document.getElementById("deleteTrackingBtn");
          ...
          // Open the modal
          const trackingModal = new bootstrap.Modal(
            ...
            try {
              ...
              if (createTracking.success) {
                ...
                const result = await DataController.updateByKey(
                  ...
                  tracking[0].id,
                  "tracking_id",
                  createTracking.data.id
                );

                if (result.status) {
                  trackingModal.hide();
                  Alert.showSuccessMessage(
                    "Tracking created and updated successfully"
                  );
                  generateTable(100, 1);
                } else {
                  Alert.showErrorMessage("Update failed");
              ...
              Alert.showErrorMessage(
                "An error occurred during tracking creation"
              );
            }
          };
          ...
          deleteTrackingBtn.onclick = async () => {
            try {
              ...
              Alert.showErrorMessage(
                "An error occurred during tracking deletion"
              );
            }
          };
        });
      ...
      tableRow.appendChild(
        Cell.createElementCell(buyerNameSpan, false, items.length, false)
      );
      tableRow.appendChild(
      ...
      tableRow.appendChild(
        Cell.createSpanCell(all_total.toFixed(2), false, items.length)
      );
      tableRow.appendChild(
    ...
    dropdownTitle.innerText = `Showing ${limit * (page - 1) + 1}-${
      limit * page
    } of ${totalCount} rows`;
...
async function generateDropdown(data) {
  try {
    ...
    const selectedFulfillmentStatus = document.getElementById(
...
const handleCreateInvoice = async (e) => {
  e.preventDefault();
  const result = await Downloader.generateInvoiceExcel(
    orders,
    toggleSpinner,
    checkboxStates
  );
...
const handleItemSummary = async (e) => {
  e.preventDefault();
  const result = await Downloader.generateItemSummaryExcel(
    orders,
    toggleSpinner,
    checkboxStates
  );
...
const handleDhlPreAlert = async (e) => {
  e.preventDefault();
  const result = await Downloader.generateDhlPreAlertExcel(
    orders,
    toggleSpinner,
    checkboxStates
  );
...
const handleDpost = async (e) => {
  e.preventDefault();
  const result = await Downloader.generateDpostExcel(
    orders,
    toggleSpinner,
    checkboxStates
  );
      ...
      if (filteredOrders.length > 0) {
        ...
        for (const filteredOrder of filteredOrders) {
          try {
            ...
            const generateBarcodeResult = (
              await ThaiPostAPIController.generateBarcode(
                filteredOrder,
                thaiPostApiHost,
                createTokenResult.access_token
              )
            ).response;
            result.generateBarcodeResult = generateBarcodeResult.fileUrl
              ? "success"
              : "failed";
            await Swal.update({ html: createResultsHTML(results) });
            if (generateBarcodeResult.fileUrl) {
              ...
              result.updateTracking = updateTracking.status
                ? "success"
                : "failed";
              await Swal.update({ html: createResultsHTML(results) });
            ...
            console.error(
              `Error processing order ${filteredOrder.details.order_id}:`,
...
const handleAftershipCSV = async (e) => {
  e.preventDefault();
  const result = await Downloader.generateAftershipCSV(
    orders,
    toggleSpinner,
    checkboxStates
  );
...
const addAllEventListeners = () => {
  ...
  const eventListeners = [
    ...
    {
      element: downloadBarcodes,
      type: "click",
      listener: handleDownloadBarcodes,
    },
...
async function filterOrdersWithBarcodes(orders) {
  ...
  for (const order of orders) {
    ...
    if (!fileExists) {
      ...
      Alert.showErrorMessage(
        `Barcode for order ${order.details.order_id} already exists.`
      );
    }
  }
...
const createResultsHTML = (results) => {
  const getStatusIcon = (status) => {
    switch (status) {
      case "success":
        return '<span class="fa-solid fa-check-circle text-success"></span>';
      case "failed":
        return '<span class="fa-solid fa-times-circle text-danger"></span>';
      case "pending":
        return '<span class="fa-solid fa-spinner fa-spin text-primary"></span>';
      default:
        return '<span class="fa-solid fa-question-circle text-warning"></span>';
    }
  };
