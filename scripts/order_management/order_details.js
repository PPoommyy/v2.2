...
if (insertOrderButton) {
  insertOrderButton.addEventListener("click", async () => {
    try {
      ...
      if (
        addressLineCount > 0 &&
        AddressController.isValidEmail(addressSplit[addressLineCount - 1][0])
      ) {
        haveEmail = true;
      ...
      if (haveEmail && addressLineCount >= 4) {
        country_code_data = await get_country_code_by_name(
          AddressController.getValueByIndex(
      ...
      const newOrder = {
        ...
        order_type_id: order_type_id
          ? order_type_id
          : country_code_data && country_code_data.data !== "TH"
          ? 1
          : 2,
      ...
      for (const item of items) {
        ...
        if (sku) {
          ...
          const existingItem = itemsList.find(
            (item) => item.sku_settings_id === id
          );

          if (existingItem && existingItem.item_price === price) {
            existingItem.quantity_purchased =
              parseInt(existingItem.quantity_purchased) + parseInt(quantity);
            existingItem.total =
              existingItem.quantity_purchased * existingItem.item_price;
          } else {
            const newItem = {
              unique_id: uniqueId,
              order_item_id: id,
...
if (saveOrderButton) {
  saveOrderButton.addEventListener("click", async () => {
    try {
      ...
      const payments_date = details.payments_date
        ? details.payments_date.split(" ")[0]
        : null;
      const date_created = details.date_created
        ? details.date_created.split(" ")[0]
        : null;
      const date_updated = details.date_updated
        ? details.date_updated.split(" ")[0]
        : null;
      ...
      to_update_order = updatedObject(
        to_update_order,
        "ship_address_1",
        details.ship_address_1,
        addressLineCount > 4 ? addressSplit[1][0] : null
      );
      to_update_order = updatedObject(
        to_update_order,
        "ship_address_2",
        details.ship_address_2,
        addressLineCount > 4 ? addressSplit[2][0] : null
      );
      to_update_order = updatedObject(
        to_update_order,
        "ship_address_3",
        details.ship_address_3,
        addressLineCount > 4 ? addressSplit[3][0] : null
      );
      to_update_order = updatedObject(
        to_update_order,
        "ship_city",
      ...
      to_update_order = updatedObject(
        ...
        isThai ? "TH" : country_code_data ? country_code_data.data : null
      );
      ...
      for (const itemInList of itemsList) {
        ...
        if (index !== -1) {
          ...
          to_update_items_object = updatedObject(
            to_update_items_object,
            "quantity_purchased",
            items[index].quantity_purchased,
            itemInList.quantity_purchased
          );
          to_update_items_object = updatedObject(
            to_update_items_object,
            "total",
      ...
      if (to_update_items && to_update_items.length > 0) {
        for (const to_update_item of to_update_items) {
          const res = await update_order_item(
            "unique_id",
            to_update_item.unique_id,
            to_update_item
          );
          if (res.status) {
            const confirmed = await swalQueue.fire({
  ...
  try {
    ...
    do {
      ...
      if (
        !orderDetails ||
        !orderDetails.data1 ||
        orderDetails.data1.items.length <= 0
      ) {
        break;
      }
    } while (true);
...
const get_order_details = async (order_id) => {
  try {
    const response = await axios.get(
      `../../backend/get/order/get_order_details.php?order_id=${order_id}`
    );
    return response.data;
  } catch (error) {
    console.error(error);
...
const get_sku_search = async (searchTerm) => {
  try {
    const response = await axios.get(
      `../../backend/get/sku/get_sku_search.php?searchTerm=${searchTerm}`
    );
    return response.data;
  } catch (error) {
    console.error(error);
...
const get_country_code_by_name = async (name) => {
  try {
    const response = await axios.get(
      `../../backend/get/get_country_code_by_name.php?name=${name}`
    );
    return response.data;
  } catch (error) {
    console.error(error);
...
const get_last_timesort = async (yearAndMonth) => {
  try {
    const response = await axios.get(
      `../../backend/get/get_last_timesort.php?year_and_month=${yearAndMonth}`
    );
...
const generateTable = async (limit, page) => {
  try {
    ...
    const tableHeader = document.createElement("thead");
    const tableHeaderRow = document.createElement("tr");
    tableHeaderRow.innerHTML = `<th>TIME SORT</th>
         <th>Detail</th>
         <th>Buyer Name</th>
    ...
    orders.forEach((order) => {
      const tableRow = document.createElement("tr");
      tableRow.innerHTML = `<td rowspan=${order.items.length}>${order.details.timesort}</td>
      ...
      order.items.slice(1).forEach((item) => {
        ...
        itemRow.innerHTML = `<td>${item.report_product_name}</td>`;
...
const generateDropdown = async (order_id, request_id) => {
  try {
    ...
    if (order_id || request_id) {
      ...
      const orderStatusButton = document.getElementById(
