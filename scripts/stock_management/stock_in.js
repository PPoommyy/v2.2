    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
          ...
          const quantity_to_add = parseInt(row.getCell(2).value);

          if (!order_product_name || !quantity_to_add || isNaN(quantity_to_add))
            continue;

          const skuData = await get_sku_search(order_product_name);
          if (!skuData || skuData.data.length === 0) {
            Alert.showErrorMessage(`❌ ไม่พบสินค้า: ${order_product_name}`);
            continue;
          }

          const product = skuData.data[0];
...
const handleTemplateDownload = async () => {
  try {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Stock In Template");

    // Add headers
    worksheet.addRow(["Product SKU", "Quantity"]);
    ...
    Downloader.downloadBlob(blob, "Stock-In-Template.csv");
...
insertStockButton.addEventListener("click", async function (event) {
  ...
  try {
    ...
    console.error(error);
    Alert.showErrorMessage("❌ ล้มเหลวในการเพิ่มสินค้าเข้าสต็อก", "danger");
  } finally {
    toggleSpinner(false);
  }
});
