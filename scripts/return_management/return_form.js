...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    let formData = {
      ...
      /* order_date: formatDateTimeWithSeconds(
        document.getElementById("orderDate").value
      ), */
      request_reason:
        requestReason.value === "Other"
  ...
  async function sendData(insertedData, table) {
    try {
      const checkTrackingNumber = await DataController.selectByKey(
        "tracking",
        "tracking_number",
        insertedData.tracking_number
      );
      if (
        !checkTrackingNumber.status ||
        checkTrackingNumber.status?.length <= 0
      ) {
        Alert.showErrorMessage("Tracking number not found.");
        return;
      }
      const checkOrderNumber = await DataController.selectByKey(
        "orders",
        "timesort",
        parseInt(insertedData.order_number)
      );
      if (!checkOrderNumber.status || checkOrderNumber.status?.length <= 0) {
        Alert.showErrorMessage("Order number not found.");
        return;
      }
